...
import { SidebarFilterButton } from "@/components/organisms/SidebarFilterButton";
...
export const SidebarFilterSection = ({
    ...
    <div className="flex-center py-4 clickble  px-4  " onClick={()=>{s__isOpen(!isOpen)}}>
        <div className=" pr-3  show-lg_x"></div>
        <div className="px-1 tx-center tx-lg opaci-hov--50">{theIcon}</div>
        <div className="flex-1 pl-3 show-lg_x w-min-200px">{filterSection.filter.title}</div>
        {isOpen && <div className="flex-1 pl-3 show-xs_lg w-min-100px">{filterSection.filter.title}</div>}
        <div className=" tx-center   tx-mdl show-lg_x" >{!isOpen ? BsChevronDown({}) : BsChevronUp({})}</div>
    </div>
